...
function pageHeader(doc: jsPDF, title: string) {
  ...
  doc.text("FIRECHECK", PAGE.margin, 10.5);
  doc.setFont("helvetica", "normal");
  doc.setTextColor(200, 204, 208);
  doc.text(title, PAGE.width - PAGE.margin, 10.5, { align: "right", maxWidth: 110 });
  doc.setFillColor(...ORANGE);
  doc.rect(0, 16, PAGE.width, 1.5, "F");
}
...
function recordHeading(doc: jsPDF, row: PdfRow, index: number, y: number) {
  ...
  doc.roundedRect(PAGE.margin, y, CONTENT_WIDTH, 14, 3, 3, "F");
  doc.setFillColor(...ORANGE);
  doc.roundedRect(PAGE.margin, y, 3, 14, 1.5, 1.5, "F");
  doc.setFont("helvetica", "bold");
  doc.setFontSize(11);
  color(doc, GRAPHITE);
  doc.text(code ? `Registro ${index + 1} · ${text(code)}` : `Registro ${index + 1}`, PAGE.margin + 8, y + 9);
  if (row.Status) {
    doc.setFontSize(8);
    color(doc, statusColor(text(row.Status)));
    doc.text(text(row.Status).toUpperCase(), PAGE.width - PAGE.margin - 4, y + 9, { align: "right" });
  }
}
...
function drawShortFields(
  ...
  let y = startY;
  const width = (CONTENT_WIDTH - 8) / 2;
  for (let index = 0; index < entries.length; index += 2) {
    const pair = entries.slice(index, index + 2);
    const lines = pair.map(([, value]) => doc.splitTextToSize(text(value), width - 4) as string[]);
    const height = Math.max(...lines.map((value) => value.length * 4.2 + 8), 14);
    y = ensure(doc, y, height + 2, title);
    pair.forEach(([label, value], pairIndex) => {
      const x = PAGE.margin + pairIndex * (width + 8);
      doc.setFont("helvetica", "bold");
      doc.setFontSize(7);
      color(doc, MUTED);
      ...
      doc.setFontSize(9);
      color(doc, GRAPHITE);
      doc.text(doc.splitTextToSize(text(value), width - 4) as string[], x, y + 8);
    });
    doc.setDrawColor(...BORDER);
...
function drawLongField(doc: jsPDF, label: string, value: PdfCell, startY: number, title: string): number {
  let y = ensure(doc, startY, 18, title);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(7);
  color(doc, MUTED);
...
function record(doc: jsPDF, row: PdfRow, index: number, startY: number, title: string): number {
  let y = ensure(doc, startY, 20, title);
...
function section(doc: jsPDF, value: PdfSection, title: string) {
  ...
  doc.setFontSize(20);
  color(doc, GRAPHITE);
  doc.text(value.title, PAGE.margin, 34);
  doc.setFont("helvetica", "normal");
  doc.setFontSize(9);
  color(doc, MUTED);
  doc.text(`${value.rows.length} registros`, PAGE.margin, 42);
  doc.setFillColor(...ORANGE);
  doc.rect(PAGE.margin, 47, 28, 2, "F");
  let y = 58;
